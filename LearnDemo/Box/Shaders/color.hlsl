...
// 常量缓存区
/*
    cbuffer对象（常量缓冲区）的名称为cbPerObject，其中存储的是一个[插图]矩阵gWorldViewProj，
    表示把一个点从局部空间变换到齐次裁剪空间所用到的由世界、视图和投影3种变换组合而成的矩阵。
    **********
    (齐次裁剪空间 == 局部空间通过MVP变换后的空间, 在齐次裁剪空间经历裁剪后进入光栅化阶段,
    光栅化执行视口变化(硬件通过透视除法，将顶点从齐次裁剪空间转到规格化设备(NDC)空间)，背面剔除,定点属性插值等后续操作)
    ********** 
    在HLSL中，可将一个[插图]矩阵声明为内置的float4x4类型。相应地，要声明[插图]矩阵或[插图]矩阵，即可分别使用float3x4和float2x4这两种类型。
    与顶点缓冲区和索引缓冲区不同的是，常量缓冲区通常由CPU每帧更新一次。举个例子，如果摄像机每帧都在不停地移动，
    那么常量缓冲区也需要在每一帧都随之以新的视图矩阵而更新。所以，我们会把常量缓冲区创建到一个上传堆而非默认堆中，这样做能使我们从CPU端更新常量。

    *** 更多常量缓存区要点请转至:BoxApp.cpp(ObjectConstants) 和 UploadBuffer.h文件中查看
*/
