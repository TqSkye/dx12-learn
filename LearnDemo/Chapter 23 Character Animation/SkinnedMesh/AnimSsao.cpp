...
UINT AnimSsao::SsaoMapWidth()const
{
    return mRenderTargetWidth / 2;
}

UINT AnimSsao::SsaoMapHeight()const
{
    return mRenderTargetHeight / 2;
}

void AnimSsao::GetOffsetVectors(DirectX::XMFLOAT4 offsets[14])
{
    std::copy(&mOffsets[0], &mOffsets[14], &offsets[0]);
}

std::vector<float> AnimSsao::CalcGaussWeights(float sigma)
{
...
ID3D12Resource* AnimSsao::NormalMap()
{
    return mNormalMap.Get();
}

ID3D12Resource* AnimSsao::AmbientMap()
{
    return mAmbientMap0.Get();
}

CD3DX12_CPU_DESCRIPTOR_HANDLE AnimSsao::NormalMapRtv()const
{
    return mhNormalMapCpuRtv;
}

CD3DX12_GPU_DESCRIPTOR_HANDLE AnimSsao::NormalMapSrv()const
{
    return mhNormalMapGpuSrv;
}

CD3DX12_GPU_DESCRIPTOR_HANDLE AnimSsao::AmbientMapSrv()const
{
    return mhAmbientMap0GpuSrv;
}

void AnimSsao::BuildDescriptors(
...
void AnimSsao::RebuildDescriptors(ID3D12Resource* depthStencilBuffer)
{
...
void AnimSsao::SetPSOs(ID3D12PipelineState* ssaoPso, ID3D12PipelineState* ssaoBlurPso)
{
    mSsaoPso = ssaoPso;
    mBlurPso = ssaoBlurPso;
}

void AnimSsao::OnResize(UINT newWidth, UINT newHeight)
{
...
void AnimSsao::ComputeSsao(
    ID3D12GraphicsCommandList* cmdList,
    AnimFrameResource* currFrame, 
    int blurCount)
{
...
void AnimSsao::BlurAmbientMap(ID3D12GraphicsCommandList* cmdList, AnimFrameResource* currFrame, int blurCount)
{
...
void AnimSsao::BlurAmbientMap(ID3D12GraphicsCommandList* cmdList, bool horzBlur)
{
...
void AnimSsao::BuildResources()
{
    ...
    texDesc.MipLevels = 1;
    texDesc.Format = AnimSsao::NormalMapFormat;
    ...
    texDesc.Format = AnimSsao::AmbientMapFormat;
...
void AnimSsao::BuildRandomVectorTexture(ID3D12GraphicsCommandList* cmdList)
{
    D3D12_RESOURCE_DESC texDesc;
    ZeroMemory(&texDesc, sizeof(D3D12_RESOURCE_DESC));
    texDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
...
void AnimSsao::BuildOffsetVectors()
{
