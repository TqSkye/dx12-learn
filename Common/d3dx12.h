...
//------------------------------------------------------------------------------------------------
/*
可以看到，CD3DX12_RESOURCE_BARRIER继承自D3D12_RESOURCE_BARRIER结构体，并添加了一些辅助方法。
Direct3D 12中的许多结构体都有其对应的扩展辅助结构变体（variation），考虑到使用上的方便性，
我们更偏爱于运用那些变体。以CD3DX12作为前缀的变体全都定义在d3dx12.h头文件当中，
这个文件并不属于DirectX 12 SDK的核心部分，但是可以通过微软的官方网站下载获得。
为了方便起见，本书源代码的Common目录里附有一份d3dx12.h头文件。
*/

/*
    资源转换:
    为了实现常见的渲染效果，我们经常会通过GPU对某个资源[插图]按顺序进行先写后读这两种操作。
    然而，当GPU的写操作还没有完成抑或甚至还没有开始，却开始读取资源，
    便会导致资源冒险（resource hazard）。为此，Direct3D专门针对资源设计了一组相关状态。
    资源在创建伊始会处于默认状态，该状态将一直持续到应用程序通过Direct3D将其转换（transition）为另一种状态为止。
    这就使GPU能够针对资源状态转换与防止资源冒险作出适当的行为。
    例如，如果要对某个资源（比如纹理）执行写操作时，需要将它的状态转换为渲染目标状态；
    而要对该纹理进行读操作时，再把它的状态变为着色器资源状态。根据Direct3D给出的转换信息，
    GPU就可以采取适当的措施避免资源冒险的发生。譬如，在读取某个资源之前，资源转换所带来的负荷会造成程序性能的下降。
    除此之外，一个自动跟踪状态转换的系统也在强行增加程序的额外开销[28]。

    通过命令列表设置转换资源屏障（transition resource barrier）数组，即可指定资源的转换；
    当我们希望以一次API调用来转换多个资源的时候，这种数组就派上了用场。
    在代码中，资源屏障用D3D12_RESOURCE_BARRIER结构体来表示[29]。
    下列辅助函数（定义于d3dx12.h头文件之中）
    将根据用户给出的资源和指定的前后转换状态，返回对应的转换资源屏障描述：
*/
